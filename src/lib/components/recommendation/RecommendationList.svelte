<script lang="ts">
  ...
  import type { RecommendationPhase } from "../../types";
  import RecommendationCard from "./RecommendationCard.svelte";
  import { recommendationPhases, recommendationStore } from "$lib/stores/recommendations.svelte";
  ...
  onMount(async () => {
    await recommendationStore.ensureLoaded();
  });

  function handleCancel() {
    recommendationStore.cancel();
  }

  async function handleRetry() {
    await recommendationStore.loadRecommendations(true);
  }
  ...
  let elapsedSeconds = $derived(
    recommendationStore.loadState.status === "loading"
      ? Math.floor((Date.now() - recommendationStore.loadState.startedAt) / 1000)
      : 0,
  );
</script>

<div class="recommendation-list">
  {#if recommendationStore.loadState.status === "loading"}
    <!-- Loading with Progress -->
    <div class="loading-container">
      <div class="loading-spinner">
        <i class="fa-solid fa-spinner fa-spin"></i>
      </div>
      <p class="loading-phase">{getPhaseText(recommendationStore.loadState.phase)}</p>
      <div class="loading-progress">
        <div class="progress-bar">
          {#each recommendationPhases as phase, i (phase)}
            <div
              class="progress-step"
              class:active={recommendationPhases.indexOf(recommendationStore.loadState.phase) >= i}
              class:current={recommendationStore.loadState.phase === phase}
            ></div>
          {/each}
        </div>
  ...
  {:else if recommendationStore.loadState.status === "error"}
    <!-- Error State -->
    <div class="error-container">
      <i class="fa-solid fa-exclamation-triangle"></i>
      <p class="error-title">{$_("recommendations.error.title") || "Fehler beim Laden"}</p>
      <p class="error-message">{recommendationStore.loadState.message}</p>
      <code class="error-code">{recommendationStore.loadState.code}</code>
      {#if recommendationStore.loadState.retryable}
  ...
  {:else if recommendationStore.loadState.status === "timeout"}
    <!-- Timeout State -->
    <div class="timeout-container">
      ...
      <p class="timeout-message">
        ...
        ({(recommendationStore.loadState.elapsedMs / 1000).toFixed(1)}s)
      </p>
      <button type="button" class="retry-btn" onclick={handleRetry}>
        <i class="fa-solid fa-arrows-rotate"></i>
        {$_("common.retry") || "Erneut versuchen"}
  ...
  {:else if recommendationStore.loadState.status === "cancelled"}
    <!-- Cancelled State -->
  ...
  {:else if recommendationStore.loadState.status === "empty"}
    <!-- Empty State -->
    <div class="empty-state">
      ...
      <p class="empty-reason">{getEmptyReasonText(recommendationStore.loadState.reason)}</p>
      ...
      {#if recommendationStore.loadState.stats}
        <div class="empty-progress">
          <div class="progress-item">
            <span class="progress-label"
              >{$_("recommendations.stats.articles_read") || "Gelesene Artikel"}</span
            >
            <span class="progress-value">{recommendationStore.loadState.stats.articles_read}</span>
          </div>
          <div class="progress-item">
            <span class="progress-label"
            ...
            >
            <span
              class="progress-value profile-{recommendationStore.loadState.stats.profile_strength.toLowerCase()}"
            >
              {recommendationStore.loadState.stats.profile_strength}
            </span>
          </div>
        </div>
      {/if}
    </div>
  {:else if recommendationStore.loadState.status === "success" || recommendationStore.recommendations.length > 0}
    <!-- Success State -->

    <!-- Stats Bar -->
    {#if recommendationStore.stats && recommendationStore.stats.articles_read > 0}
      <div class="stats-bar">
        <div class="stat-item">
          <i class="fa-solid fa-bookmark"></i>
          <span
            >{recommendationStore.stats.total_saved}
            {$_("recommendations.stats.articles_saved") || "gespeichert"}</span
          >
        </div>
        <div class="stat-item">
          <span
            class="profile-badge profile-{recommendationStore.stats.profile_strength.toLowerCase()}"
          >
            {recommendationStore.stats.profile_strength}
          </span>
        </div>
        {#if recommendationStore.loadTimingMs}
          <div class="stat-item timing">
            <i class="fa-solid fa-stopwatch"></i>
            <span>{(recommendationStore.loadTimingMs / 1000).toFixed(2)}s</span>
          </div>
        {/if}
      </div>
    {/if}
    ...
    <div class="recommendations-grid">
      {#each recommendationStore.recommendations as recommendation (recommendation.fnord_id)}
        <RecommendationCard
          {recommendation}
          onsave={(fnordId) => recommendationStore.save(fnordId)}
          onunsave={(fnordId) => recommendationStore.unsave(fnordId)}
          onclick={handleClick}
        />
      {/each}
    </div>
  ...
  {#if showDiagnostics}
    <div class="diagnostics-panel">
      ...
      <dl>
        <dt>Request ID</dt>
        <dd><code>{recommendationStore.requestId || "none"}</code></dd>

        <dt>Status</dt>
        <dd><code>{recommendationStore.loadState.status}</code></dd>

        <dt>Load Time</dt>
        <dd>
          {recommendationStore.loadTimingMs ? `${recommendationStore.loadTimingMs}ms` : "N/A"}
        </dd>

        {#if recommendationStore.stats}
          <dt>Articles Read</dt>
          <dd>{recommendationStore.stats.articles_read}</dd>

          <dt>With Embedding</dt>
          <dd>{recommendationStore.stats.articles_with_embedding}</dd>

          <dt>Candidate Pool</dt>
          <dd>{recommendationStore.stats.candidate_pool_size}</dd>

          <dt>Top Keywords</dt>
          <dd>
            {#if recommendationStore.stats.top_keywords.length > 0}
              {recommendationStore.stats.top_keywords
                .slice(0, 5)
                .map((k) => k.name)
                .join(", ")}
            {:else}
