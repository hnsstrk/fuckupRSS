<script lang="ts">
  ...
  async function loadStats() {
    ...
    try {
      // Load keyword stats
      const status = await invoke<{ queue_size: number; worker_running: boolean }>(
        "get_embedding_queue_status",
      );
      const stats = await invoke<{ total_keywords: number; total_connections: number }>(
        "get_network_stats",
      );
  ...
  let syncStatus = $derived(
    appState.syncing ? "syncing" : appState.lastSyncResult ? "done" : "idle",
  );

  let batchStatus = $derived(appState.batchProcessing ? "processing" : "idle");

  let embeddingStatus = $derived(
    appState.embeddingProgress?.is_processing
      ? "processing"
      : keywordStats.queue_size > 0
        ? "queued"
        : "idle",
  );

  let totalVram = $derived(loadedModels.reduce((sum, m) => sum + m.size_vram, 0));

  let totalArticles = $derived(appState.pentacles.reduce((sum, p) => sum + p.article_count, 0));
</script>
...
<footer class="status-bar">
  ...
  <div class="status-section" title="Feed Sync">
    <i
      class="status-icon {syncStatus === 'syncing'
  ...
  <div class="status-section ollama-section" title={$_("statusBar.ollamaTooltip")}>
    {#if appState.hasAnyMissingModel}
      ...
      <span
        ...
        title={appState.missingMainModel || appState.missingEmbeddingModel || ""}
      >
    ...
    {:else if loadedModels.length > 0}
      ...
      <span class="status-value models">
        {#each loadedModels as model, i (model.name)}
          <span class="model-name" title="{model.name} ({model.parameter_size})"
