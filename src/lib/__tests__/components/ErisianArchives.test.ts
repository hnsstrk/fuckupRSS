...
describe('ErisianArchives', () => {
  ...
  describe('Failed Tab', () => {
    ...
    it('maps AnalysisStatusArticle to Fnord-like structure', async () => {
      ...
      const failedArticles = await invoke<typeof mockFailedArticle[]>('get_failed_articles', { limit: 50, offset: 0 });
  ...
  describe('Tab Filter Auto-Remove Logic', () => {
    ...
    it('updates local article list when status changes', () => {
      ...
      const updateArticleStatus = (articles: Article[], id: number, newStatus: string): Article[] => {
