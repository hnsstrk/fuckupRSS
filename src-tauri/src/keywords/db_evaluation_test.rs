...
fn load_articles_from_db(limit: usize) -> Vec<(i64, String, String)> {
    ...
    match Connection::open(db_path) {
        Ok(conn) => {
            ...
            let articles = stmt.query_map([limit], |row| {
                Ok((
                    row.get::<_, i64>(0)?,
                    row.get::<_, String>(1)?,
                    row.get::<_, String>(2)?
...
fn get_near_duplicate_pairs(keywords: &[String], max_distance: usize) -> Vec<(String, String, usize)> {
    ...
    fn test_db_keyword_extraction_evaluation() {
        ...
        for (idx, (id, title, content)) in articles.iter().take(10).enumerate() {
            ...
            println!("\n  OLD Method ({} keywords, {} near-dups, {} garbage, diversity: {:.2}):",
                old_keywords.len(), old_near_dups, old_garbage, old_diversity);
            for (i, kw) in old_keywords.iter().take(10).enumerate() {
                let markers = if contains_garbage(kw) { " [GARBAGE]" } else { "" };
            ...
            println!("\n  NEW Method ({} keywords, {} near-dups, {} garbage, diversity: {:.2}):",
                new_keywords.len(), new_near_dups, new_garbage, new_diversity);
            ...
            let improvement = if new_near_dups < old_near_dups {
                ...
                format!("REGRESSED: {} more near-duplicates", new_near_dups - old_near_dups)
            } else {
                "NO CHANGE".to_string()
            };
            println!("\n  => Assessment: {}\n", improvement);
        ...
        println!("\n==========================================================================");
        println!("  PROCESSING ALL {} ARTICLES FOR STATISTICS", articles.len());
        println!("==========================================================================\n");
        ...
        let dup_score = if new_total_near_dups == 0 { 10 }
            else if new_total_near_dups < 10 { 8 }
            else if new_total_near_dups < 30 { 6 }
            else { 4 };

        let garbage_score = if new_total_garbage == 0 { 10 }
            else if new_total_garbage < 5 { 8 }
            else if new_total_garbage < 20 { 6 }
            else { 4 };

        let avg_diversity = new_diversity_sum / article_count;
        let diversity_score = if avg_diversity > 0.6 { 9 }
            else if avg_diversity > 0.5 { 7 }
            else if avg_diversity > 0.4 { 5 }
            else { 3 };
        ...
        println!("  - Near-duplicate avoidance: {}/10 ({} total near-dups)", dup_score, new_total_near_dups);
        println!("  - Garbage filtering: {}/10 ({} garbage keywords)", garbage_score, new_total_garbage);
        println!("  - Diversity: {}/10 (avg: {:.3})", diversity_score, avg_diversity);
        println!("  - Relevance: {}/10 (estimated)", relevance_score);
        println!("  - Topic coverage: {}/10 (estimated)", coverage_score);
        println!();

        let overall = (dup_score + garbage_score + diversity_score + relevance_score + coverage_score) as f64 / 5.0;
        ...
        if old_total_near_dups > new_total_near_dups {
            println!("  [+] Near-duplicates reduced by {} ({:.1}%)",
            ...
            println!("  [=] Near-duplicates unchanged ({} total)", new_total_near_dups);
        } else {
            println!("  [-] Near-duplicates increased by {}", new_total_near_dups - old_total_near_dups);
        }

        if old_total_garbage > new_total_garbage {
            println!("  [+] Garbage keywords reduced by {}", old_total_garbage - new_total_garbage);
        } else if old_total_garbage == new_total_garbage {
            println!("  [=] Garbage keywords unchanged ({} total)", new_total_garbage);
        }

        if new_div > old_div {
            println!("  [+] Diversity improved by {:.1}%", (new_div - old_div) / old_div * 100.0);
        } else if (new_div - old_div).abs() < 0.01 {
            println!("  [=] Diversity unchanged ({:.3})", new_div);
        } else {
            println!("  [-] Diversity decreased by {:.1}%", (old_div - new_div) / old_div * 100.0);
        }
