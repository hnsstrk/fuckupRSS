...
pub fn get_known_keyword_type(keyword: &str) -> Option<&'static str> {
    ...
    if KNOWN_PERSONS
        .iter()
        .any(|p| p.to_lowercase() == keyword_lower)
    {
        return Some("person");
    }
    ...
    if KNOWN_ORGANIZATIONS
        .iter()
        .any(|o| o.to_lowercase() == keyword_lower)
    {
    ...
    if KNOWN_SPORTS
        .iter()
        .any(|s| s.to_lowercase() == keyword_lower)
    {
        return Some("organization");
    }
    ...
    if KNOWN_LOCATIONS
        .iter()
        .any(|l| l.to_lowercase() == keyword_lower)
    {
    ...
    if KNOWN_CONCEPTS
        .iter()
        .any(|c| c.to_lowercase() == keyword_lower)
    {
        return Some("concept");
    }
...
mod tests {
    ...
    fn test_known_keyword_type() {
        ...
        // Organizations
        assert_eq!(
            get_known_keyword_type("Deutsche Bank"),
            Some("organization")
        );
        // Sports teams (also organizations)
        assert_eq!(
            get_known_keyword_type("Bayern München"),
            Some("organization")
        );
        ...
        assert_eq!(
            get_known_keyword_type("Künstliche Intelligenz"),
            Some("concept")
        );
        // Unknown
    ...
    fn test_seed_data_counts() {
        ...
        assert!(
            KNOWN_ORGANIZATIONS.len() > 50,
            "Should have many known organizations"
        );
        assert!(
            KNOWN_LOCATIONS.len() > 100,
            "Should have many known locations"
        );
        assert!(
            KNOWN_ACRONYMS.len() > 100,
            "Should have many known acronyms"
        );
        assert!(
            KNOWN_CONCEPTS.len() > 150,
            "Should have many known concepts"
        );
        assert!(
            KNOWN_SPORTS.len() > 30,
            "Should have many known sports teams"
        );
    }
}
